      ...
      <div className="h-full md:min-h-screen flex items-center justify-center bg-primary p-4">
        ...
        <div className="relative w-[500px] h-[600px] flex items-center justify-center rounded-full overflow-hidden">
          ...
          {[...Array(38)].map((_, i) => {
            ...
            const rotation = Math.round((-145 + i * (290 / 38)) * 100) / 100;
          ...
          <div className="absolute w-4/5 max-w-[400px] z-10 p-5 rounded-3xl">
            <form className="w-full px-2.5">
              ...
              {/* Email Input */}
              <FloatingLabelInput type="email" label="Nueva contraseña" required />

              {/* Password Input */}
              <FloatingLabelInput type="password" label="Confirmar contraseña" required />

              {/* Login Button */}
              <button
                type="submit"
                className="w-full h-11 mt-4 bg-accent border-none outline-none rounded-full cursor-pointer text-base text-white font-semibold hover:bg-accent/90 transition-all"
              >
                Confirmar
              </button>
